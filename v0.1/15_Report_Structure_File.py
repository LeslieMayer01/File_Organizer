...
def verificar_estructura_carpeta(ruta, carpeta_principal, nombre_indice, segundo_archivo_str):
    errores = []
    ...
    if not os.path.exists(ruta_principal):
        ...
        if len(archivos) < 2:
            ...
            if not archivos[0].startswith(nombre_indice):
                errores.append((archivos[0], "Archivo", os.path.join(ruta_principal, archivos[0])))

            # Verificar segundo archivo
            if not archivos[1].startswith(segundo_archivo_str):
                errores.append(("error primer digito", "Archivo", os.path.join(ruta_principal, archivos[1])))
        ...
        for carpeta in carpetas:
            if carpeta.startswith("056314"):
                ...
                reporte_errores.extend(
                    verificar_estructura_carpeta(ruta_carpeta, "01PrimeraInstancia/C01Principal", 
                                                 "00IndiceElectronicoC01", "01")
                )
                
                # Revisar segunda estructura: 01PrimeraInstancia/C05MedidasCautelares
                reporte_errores.extend(
                    verificar_estructura_carpeta(ruta_carpeta, "01PrimeraInstancia/C05MedidasCautelares", 
                                                 "00IndiceElectronicoC05", "01")
                )
