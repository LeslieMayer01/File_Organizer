...
def main():
    ...
    guardar_csv(
        "reporte_todas_carpetas.csv", carpetas_todas, ["nombre carpeta", "ruta"]
    )

    # Reporte 2: Nombres únicos sin carpetas '202*'
    carpetas_unicas = obtener_nombres_unicos_excluyendo_202(ruta_objetivo)
    guardar_csv(
        "reporte_nombres_unicos.csv",
        carpetas_unicas.items(),
        ["nombre carpeta", "ruta"],
    )
    ...
    guardar_csv(
        "reporte_extensiones.csv",
        extensiones.items(),
        ["extension_archivo", "cantidad"],
    )
