        ...
        for file in files:
            ...
            if extension == "":
                ...
                archivos_sin_extension.append(
                    {"nombre_archivo": nombre, "ruta": os.path.join(root, file)}
                )
...
def renombrar_archivos_sin_extension(archivos):
    for archivo in archivos:
        ...
        nuevo_nombre = (
            f"{archivo['nombre_archivo']}.pdf"  # Asigna la nueva extensión .pdf
        )
        nueva_ruta = os.path.join(
            os.path.dirname(ruta_actual), nuevo_nombre
        )  # Nueva ruta con la nueva extensión
...
if __name__ == "__main__":
    ...
    ruta_reporte = "./reports/17_" + añadir_fecha_y_hora_al_nombre(
        "_Files_Without_Extensions.xlsx"
    )
