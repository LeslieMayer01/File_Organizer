    ...
    with open(ruta_csv, mode="r", encoding="utf-8") as archivo:
        ...
        for fila in lector:
            if fila:  # Verifica que la fila no esté vacía
                diccionario.add(
                    fila[0].strip().lower()
                )  # Convertir a minúsculas y agregar al diccionario
    return diccionario
...
def eliminar_palabras(nombre):
    ...
    palabras = nombre.split()
    nombre_filtrado = [
        palabra for palabra in palabras if palabra.lower() not in palabras_a_eliminar
    ]
...
def formatear_fechas(nombre):
    ...
    def reemplazo_fecha(match):
        ...
        if len(año) == 2:
            año = (
                "20" + año
            )  # Asume que las fechas son de este siglo si son de dos dígitos
        dia = dia.zfill(2)
        mes = mes.zfill(2)
...
def procesar_nombre_archivo(nombre):
    ...
    nombre_sin_extension = capitalizar_palabras(
        nombre_sin_extension, diccionario_palabras
    )
...
def renombrar_archivos_recursivamente(ruta):
    ...
    if conflictos:
        with open(
            "./reports/03_Manual_Files_To_Check_3.csv",
            "w",
            newline="",
            encoding="utf-8",
        ) as csvfile:
