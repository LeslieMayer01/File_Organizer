...
def procesar_carpeta_instancia(ruta_base, archivo_excel):
            ...
            if dir_name.startswith("056314"):
                ...
                if os.path.exists(ruta_unica_instancia):
                    ...
                    if not os.path.exists(ruta_primera_instancia):
                        ...
                        print(
                            f"Carpeta renombrada: {ruta_unica_instancia} a {ruta_primera_instancia}"
                        )
                    else:
                        ...
                        nombre_nuevo = "01PrimeraInstancia"
                        print(
                            f"No se pudo renombrar {ruta_unica_instancia}, ya existe {ruta_primera_instancia}"
                        )
                else:
                    ...
                    if not os.path.exists(ruta_primera_instancia):
                        ...
                        nombre_nuevo = "01PrimeraInstancia"
                        print(
                            f"La carpeta {ruta_primera_instancia} ya existía, no fue necesario crearla."
                        )
                ...
                datos_carpetas.append(
                    {
                        ...
                        "Estado": estado,
                        "Ruta": (
                            ruta_primera_instancia
                            if estado in ["CREADA", "RENOMBRADA"]
                            else ruta_carpeta
                        ),
                    }
                )
    ...
    df = pd.DataFrame(
        datos_carpetas, columns=["Nombre Previo", "Nombre Nuevo", "Estado", "Ruta"]
    )
...
archivo_excel = "./reports/06_" + añadir_fecha_y_hora_al_nombre(
    "Rename_01UnicaInstancia_Report.xlsx"
)
