...
def renombrar_carpetas(ruta_base, archivo_excel):
    ...
    palabras_titulos = [
        "deposito",
        "titulo",
        "TITULO" "Deposito" "DEPOSITOS" "Titulo",
        "DJ04",
    ]
                ...
                datos_renombrados.append(
                    {
                        ...
                        "Estado": estado,
                        "Ruta Absoluta": (
                            ruta_antigua if estado == "DUPLICADA" else ruta_nueva
                        ),
                    }
                )
    ...
    df = pd.DataFrame(
        datos_renombrados,
        columns=["Nombre Anterior", "Nombre Nuevo", "Estado", "Ruta Absoluta"],
    )
...
archivo_excel = "./reports/04_" + añadir_fecha_y_hora_al_nombre(
    "Create_Folder_Report.xlsx"
)
