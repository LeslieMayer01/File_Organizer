...
def es_excel(nombre_archivo):
    ...
    return any(nombre_archivo.lower().endswith(ext) for ext in extensiones_excel)

def contiene_indice(nombre):
    """Verifica si el nombre del archivo o carpeta contiene 'indice' (sin importar mayúsculas/minúsculas)."""
...
def listar_indices_excel(ruta, salida_csv):
    """Lista archivos Excel que contienen 'indice' en el nombre y los guarda en un CSV."""
    resultados = []
...
def eliminar_indices_electronicos_y_carpetas_vacias(ruta, reporte_csv, ejecutar_eliminaciones=False):  # CAMBIAR A TRUE PARA EJECUTAR ELIMINACIÓN
    """Elimina archivos Excel con 'indice' en el nombre y carpetas vacías en la ruta, o solo genera el reporte si no se ejecutan las eliminaciones."""
    eliminados = []
...
ruta_csv_salida = obtener_nombre_reporte(base_path_reportes, 'listado_indices_excel')
reporte_eliminaciones = obtener_nombre_reporte(base_path_reportes, 'reporte_eliminaciones')

# Variable de control: Si es True, realiza eliminaciones; si es False, solo genera el reporte.
ejecutar_eliminaciones = True  # Cambia esto a False si solo deseas generar el reporte sin eliminar nada

# Ejecutar las funciones
listar_indices_excel(ruta_a_organizar, ruta_csv_salida)
eliminar_indices_electronicos_y_carpetas_vacias(ruta_a_organizar, reporte_eliminaciones, ejecutar_eliminaciones)
